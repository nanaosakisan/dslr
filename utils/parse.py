...
def check_features(content):
    ...
    for i, obj in enumerate(content):
        if obj != features[i]:
            return (
                0,
                "Erreur dans le nom de feature à la ligne : "
                + str(i)
                + ". Feature \
            attendu : "
                + features[i],
...
def check_birthday(dataset):
    for i, row in enumerate(dataset[1:]):
        try:
            ...
            return (
                0,
                "Le format de la date est incorrect à la ligne : "
                + str(i)
                + ". \
